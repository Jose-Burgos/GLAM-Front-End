...
export const uploadFileToBucket = async (bucketId, path, file) => {         // three parameters (the id of the bucket, the path in which you create the file, the file)
  try {
...
export const createEmptyFile = async (bucketId, path, mimeType, size) => {      // four parameters (id of Bucket, path of the file to be created, tipo de archivo (image/jpeg para imagenes), tamaño en bytes)
  try {
    if (!path) {
    ...
    const emptyBlob = new Blob([], { type: mimeType, endings: "transparent", size });
